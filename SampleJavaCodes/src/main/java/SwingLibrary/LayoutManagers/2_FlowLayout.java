 ...
 * We LEFT, RIGHT, CENTER, LEADING, TRAILING. For the last two, if the
 ...
 * does not ask for a row size (I think it just looks for the highest row).
 * Moreover, when the number of components exceed a row, it does not put them in
 * the next row, it rather hides them! Try and play with the size of the frame
 * size when displayed to see this effect.
 * 
 * As another example, we use a JPanel (which has FlowLayout as manager) and try
 * and add it to another JFrame. The point is that again, we'd only have one
 * row containing both the buttons and the JFrame. Moreover, we'll never see the JPanel
 * break down into two rows (try and play with frame size after compilation). Why?
